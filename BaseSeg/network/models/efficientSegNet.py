...
class EfficientSegNet(nn.Module):
    ...
    def __init__(self, cfg=None):
        ...
        self.conv0_0 = nn.Conv3d(1, num_channel[0], kernel_size=1, bias=False)
        ...
        self.trans_5 = _ConvINReLU3D(num_channel[4]+num_channel[3], num_channel[3], kernel_size=1, stride=1, padding=0, p=0.2)
        self.trans_6 = _ConvINReLU3D(num_channel[3] + num_channel[2], num_channel[2], kernel_size=1, stride=1,padding=0, p=0.2)
        self.trans_7 = _ConvINReLU3D(num_channel[2] + num_channel[1], num_channel[1], kernel_size=1, stride=1,
                                     padding=0, p=0.2)
        self.trans_8 = _ConvINReLU3D(num_channel[1] + num_channel[0], num_channel[0], kernel_size=1, stride=1,
                                     padding=0, p=0.2)
    ...
    def _mask_layer(self, block, in_channels, out_channels, num_block, stride):
        layers = []
        layers.append(block(in_channels, out_channels, p=0.2, stride=stride,
                            is_dynamic_empty_cache=self.is_dynamic_empty_cache))
        for _ in range(num_block-1):
            layers.append(block(out_channels, out_channels, p=0.2, stride=1,
                                is_dynamic_empty_cache=self.is_dynamic_empty_cache))
    ...
    def forward(self, x):
        ...
        x3_1 = self.trans_5 (torch.cat([self.up(x4_0),x3_0],dim=1))
        # x3_1 = self.up(self.trans_4(x4_0)) + x3_0
        # print('x3_1:',x3_1.shape)
        # x3_0 = self.conv3_1(self.up(self.trans_4(x4_0)) + x3_1)
        x3_0 = self.conv3_1(self.up(self.trans_4(x4_0)) + x3_1)

        # x3_0 = self.conv3_1(self.up(self.trans_4(x4_0)) + x3_0)
        # print('x3-0：', x3_0.shape)
        if self.is_dynamic_empty_cache:
            del x4_0
            torch.cuda.empty_cache()
        x2_1 = self.trans_6 (torch.cat([self.up(x3_1),x2_0],dim=1))
        # print('x2-1：', x2_1.shape)
        x2_0 = self.conv2_2(self.up(self.trans_3(x3_0)) + x2_1)

        # print('x2-0：', x2_0.shape)
        ...
        x1_1 = self.trans_7 (torch.cat([self.up(x2_1),x1_0],dim=1))
        x1_0 = self.conv1_3(self.up(self.trans_2(x2_0)) + x1_1)
        # x1_0 = self.conv1_3(self.up(self.trans_2(x2_0)) + x1_0)
        if self.is_dynamic_empty_cache:
            del x2_0
            torch.cuda.empty_cache()
        x_1 = self.trans_8 (torch.cat([self.up(x1_1),x],dim=1))
        x = self.conv0_4(self.up(self.trans_1(x1_0)) + x_1)
        # x = self.conv0_4(self.up(self.trans_1(x1_0)) + x_1)
...
if __name__ == '__main__':
    ...
    x = torch.randn(2,1,192,192,192).to(device)
    net =EfficientSegNet().to(device)
    out = net(x)
    print(out.shape)