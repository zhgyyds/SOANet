...
from ..network.blocks.prior import PriorConvBlock, ideal_affinity_matrix, prior_losses
    ...
    def _train(self, epoch):
        ...
        for index, (images, masks) in enumerate(self.train_loader):
            ...
            if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                ...
                # output_seg,map = self.model(images)   #c_prior
                output_seg = self.model(images)
                loss_func = SegLoss(loss_func=self.cfg.TRAINING.LOSS, activation=self.cfg.TRAINING.ACTIVATION)
                seg_loss = loss_func(output_seg, masks, is_average=True)

 # prior loss
 #                ideal_masks = ideal_affinity_matrix(masks)
 #                # print('masks')
 #                # print(masks.shape)
 #                # print(map)
 #                # print(map.shape)
 #                # print(ideal_masks)
 #                # print(ideal_masks.shape)

                # prior_loss = prior_losses(map,ideal_masks)
                # # print(prior_loss)
                # seg_loss = seg_loss + 10*prior_loss
                # # print(seg_loss)
        ...
        for images, masks in self.val_loader:
            ...
            with torch.no_grad():
                if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                    ...
                    # output_seg,map = self.model(images)     # prior loss
                    output_seg = self.model(images)
    ...
    def do_test(self, fold=1):
        ...
        for batch_idx, data_dict in enumerate(test_dataset):
            ...
            with torch.no_grad():
                # crop_image,map = self.fine_model(crop_image)
